...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* tooltip — shows on hover of any element with [data-tooltip] */
  ...
  [data-tooltip] em { color: var(--accent-strong); font-style: italic; }
  [data-tooltip] strong { color: var(--text); font-weight: 500; }
...
<header class="topbar">
  ...
  <div class="controls">
    ...
    <div class="toggle" id="value-mode-toggle" data-tooltip="annualizzato (predefinito): appalti pluriennali ripartiti sulla durata; cap a €100 mln/cig per filtrare typo evidenti nei dati anac. mostra il flusso reale del 2024 (€133 mld).&#10;&#10;nominale: valore totale dell'appalto contato per intero nell'anno di aggiudicazione, senza cap. coincide con la metodologia ufficiale anac (€296 mld vs anac €272 mld nel 2024). include anche le anomalie/typo della fonte anac (es. €14 mld per un anno di gas).">
...
(async function(){
  ...
  function renderPopup(m){
    ...
    const vk = valKey();
    const vIdx = currentValueMode === 'vn' ? 2 : 1;
    // Each op row: [name, v_annual, vn_nominal, count, rti]
    const sortIdx = currentMetric === 'v' ? vIdx : 3;
    const list = b.o.slice().sort((a,bb) => bb[sortIdx] - a[sortIdx]).slice(0, 5);
    const totalForShare = currentMetric==='v' ? b[vk] : b.c;
    const vmodeLabel = currentValueMode === 'vn' ? 'nominale' : 'annualizzato';
    ...
    html += '<div class="meta">' + metaParts.join(' · ') + '<br><em>' + esc(catLabel) + '</em> · valore ' + vmodeLabel + '</div>';
    html += '<div class="tot"><span>valore totale aggiudicato</span><strong>' + eur(b[vk]) + '</strong></div>';
    ...
    for (const row of list){
      const [nm, val_a, val_n, cnt, rti] = row;
      const val = currentValueMode === 'vn' ? val_n : val_a;
    ...
    if (b.cap && b.cap > 0) {
      html += '<div class="ih-note" style="background:rgba(212,165,116,0.15);color:var(--amber)">' + b.cap + ' contratto' + (b.cap===1?'':'i') + ' in questa categoria conteneva valori anomali (es. typo di registrazione) ed è stato limitato a € 100 mln per cig.</div>';
    }
  ...
  restyleAll();
  // Defensive: re-render after a tick in case the map container sized
  // differently before fonts/styles settled.
  setTimeout(() => { map.invalidateSize(); cluster.refreshClusters(); }, 200);
})();
</script>
</body>
</html>"""
...
(async function(){
  ...
  // Render list — by current filter + search + sort metric + value mode
  let currentFilter = 'ALL';
  let currentSearch = '';
  let currentSort = 'v';            // 'v' value | 'c' count
  let currentValueMode = 'v';       // 'v' annualized | 'vn' nominal
  ...
  function filteredSorted() {
    ...
    const vk = valKey();
    // CPV breakdown per op: cpv[code] = [v_annual, vn_nominal, count]
    const cpvValIdx = currentValueMode === 'vn' ? 1 : 0;
    if (currentFilter !== 'ALL') {
      arr = arr.filter(o => o.cpv && o.cpv[currentFilter]);
      arr = arr.map(o => ({ ...o, _filt_v: o.cpv[currentFilter][cpvValIdx], _filt_c: o.cpv[currentFilter][2] }));
      arr.sort((a,b) => currentSort === 'v' ? b._filt_v - a._filt_v : b._filt_c - a._filt_c);
    } else {
      arr = arr.slice();
      arr.sort((a,b) => currentSort === 'v' ? b[vk] - a[vk] : b.c - a.c);
    }
    return arr;
  }
  ...
  function renderList(){
    ...
    list.innerHTML = shown.map(o => {
      const v = currentFilter==='ALL' ? o[vk] : o._filt_v;
  ...
  function renderDetail(o) {
    ...
    metaParts.push('valore ' + (currentValueMode === 'vn' ? 'nominale' : 'annualizzato'));
    html += '<div class="op-meta">' + metaParts.join(' · ') + '</div>';

    // Stats grid
    const totV = o[vk];
    html += '<div class="stats-grid">';
    html += '<div class="stat-card"><div class="label">valore totale</div><div class="value">' + eurShort(totV) + '</div><div class="sub">€ ' + itf.format(totV) + '</div></div>';
    html += '<div class="stat-card"><div class="label">contratti</div><div class="value">' + itf0.format(o.c) + '</div><div class="sub">media ' + eurShort(totV/o.c) + '</div></div>';
    ...
    // Concentration row (computed on annualized regardless of mode — see methodology note)
    ...
    // Re-sort SAs by the active value field
    const sasSorted = o.sas.slice().sort((a,b) => (b[vk]||0) - (a[vk]||0));
    for (const sa of sasSorted) {
      const saV = sa[vk] || 0;
      html += '<div class="sa-row">' +
        ...
        '</div>' +
        '<div class="sa-value">' + eurShort(saV) + '<span class="count">' + itf0.format(sa.c) + ' contratti</span></div>' +
      '</div>';
    }
    html += '</div>';
    ...
    if (o.cpv && Object.keys(o.cpv).length > 0) {
      ...
      const cpvEntries = Object.entries(o.cpv).sort((a,b) => b[1][cpvValIdx] - a[1][cpvValIdx]);
      const totalShownV = cpvEntries.reduce((s, [_, arr]) => s + arr[cpvValIdx], 0);
      for (const [code, arr] of cpvEntries) {
        const v = arr[cpvValIdx]; const c = arr[2];
    ...
    if (miniContainer) {
      ...
      const maxV = Math.max(...o.sas.map(s => s[vk] || 0)) || 1;
      const bounds = [];
      for (const sa of o.sas) {
        const saV = sa[vk] || 0;
        const r = 5 + Math.sqrt(saV / maxV) * 18;
        const m = L.circleMarker([sa.la, sa.lo], {
          radius: r, color: '#0f172a', weight: 0.8, fillColor: '#ffffff', fillOpacity: 0.95
        });
        m.bindTooltip(sa.n + '<br>' + eurShort(saV) + ' · ' + sa.c + ' contratti', { direction: 'top', offset: [0, -2], className: 'place-label-cap' });
        m.addTo(map);
  ...
  document.querySelectorAll('#value-mode-toggle button').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      currentValueMode = btn.dataset.vm;
      renderList();
      if (selectedKey) renderDetail(OPS[selectedKey]);
    });
  });
